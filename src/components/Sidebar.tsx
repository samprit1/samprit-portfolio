...
export function Sidebar({open,close,contactEnabled=true}:{open:boolean;close:()=>void;contactEnabled?:boolean}){
  ...
  return <><aside className="fixed bottom-0 left-0 top-16 z-30 hidden w-60 overflow-y-auto border-r border-[var(--line)] bg-[var(--bg)] px-3 py-3 md:block">{block(primary)}<hr className="my-3 border-[var(--line)]"/><p className="px-3 py-2 text-xs font-bold text-[var(--muted)]">PORTFOLIO</p>{block(portfolio)}{desktopMore}<hr className="my-3 border-[var(--line)]"/><p className="px-3 py-2 text-xs font-bold text-[var(--muted)]">CONNECT</p>{block(links)}</aside><div className={`fixed inset-0 z-50 md:hidden ${open?'':'pointer-events-none'}`}><div onClick={close} className={`absolute inset-0 bg-black/60 transition-opacity ${open?'opacity-100':'opacity-0'}`}/><aside className={`absolute bottom-0 left-0 top-0 w-72 overflow-auto bg-[var(--bg)] p-3 transition-transform ${open?'translate-x-0':'-translate-x-full'}`}><div className="mb-4 flex items-center justify-between px-2"><b>SAMPRIT</b><button onClick={close} className="focus rounded-full p-2"><X/></button></div>{block(primary)}<hr className="my-3 border-[var(--line)]"/><p className="px-3 py-2 text-xs font-bold text-[var(--muted)]">PORTFOLIO</p>{block(portfolio)}{mobileMore}<hr className="my-3 border-[var(--line)]"/>{block(links)}</aside></div></>
}
